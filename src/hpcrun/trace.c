...
hpcrun_trace_append(core_profile_trace_data_t *cptd, cct_node_t* node, unsigned int metric_id, uint32_t dLCA, uint64_t sampling_period)
{
  if (tracing && hpcrun_sample_prob_active()) {
    ...
    if (sampling_period > 0 && TD_GET(prev_nanotime) != 0 && nanotime - TD_GET(prev_nanotime) > TRACE_GAP_FACTOR * sampling_period) {
      ...
      hpcrun_trace_append_with_time_real(cptd, no_activity_call_path_id, metric_id, dLCA, TD_GET(prev_nanotime) + sampling_period);
    }
    TD_GET(prev_nanotime) = nanotime;
