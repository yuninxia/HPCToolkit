...
ze_result_t f_zeDriverGetExtensionFunctionAddress(
    ze_driver_handle_t hDriver, const char* name, void** ppFunctionAddress,
    const struct hpcrun_foil_appdispatch_level0* dispatch) {
  return dispatch->zeDriverGetExtensionFunctionAddress(hDriver, name,
                                                       ppFunctionAddress);
}
...
ze_result_t
f_zeDeviceGetRootDevice(ze_device_handle_t hDevice, ze_device_handle_t* phRootDevice,
                        const struct hpcrun_foil_appdispatch_level0* dispatch) {
...
ze_result_t f_zetContextActivateMetricGroups(
    zet_context_handle_t hContext, ze_device_handle_t hDevice, uint32_t count,
    zet_metric_group_handle_t* phMetricGroups,
    const struct hpcrun_foil_appdispatch_level0* dispatch) {
  return dispatch->zetContextActivateMetricGroups(hContext, hDevice, count,
                                                  phMetricGroups);
}

ze_result_t
f_zetMetricStreamerOpen(zet_context_handle_t hContext, ze_device_handle_t hDevice,
                        zet_metric_group_handle_t hMetricGroup,
                        zet_metric_streamer_desc_t* desc,
                        ze_event_handle_t hNotificationEvent,
                        zet_metric_streamer_handle_t* phMetricStreamer,
                        const struct hpcrun_foil_appdispatch_level0* dispatch) {
  return dispatch->zetMetricStreamerOpen(hContext, hDevice, hMetricGroup, desc,
                                         hNotificationEvent, phMetricStreamer);
}
...
ze_result_t f_zetMetricGroupGet(zet_device_handle_t hDevice, uint32_t* pCount,
                                zet_metric_group_handle_t* phMetricGroups,
                                const struct hpcrun_foil_appdispatch_level0* dispatch) {
...
f_zetMetricStreamerReadData(zet_metric_streamer_handle_t hMetricStreamer,
                            ...
                            uint8_t* pRawData,
                            const struct hpcrun_foil_appdispatch_level0* dispatch) {
  return dispatch->zetMetricStreamerReadData(hMetricStreamer, maxReportCount,
                                             pRawDataSize, pRawData);
}
...
ze_result_t
f_zelTracerSetPrologues(zel_tracer_handle_t tracer, zel_core_callbacks_t* prologues,
                        const struct hpcrun_foil_appdispatch_level0* dispatch) {
  return dispatch->zelTracerSetPrologues(tracer, prologues);
}

ze_result_t
f_zelTracerSetEpilogues(zel_tracer_handle_t tracer, zel_core_callbacks_t* epilogues,
                        const struct hpcrun_foil_appdispatch_level0* dispatch) {
  return dispatch->zelTracerSetEpilogues(tracer, epilogues);
}

ze_result_t
f_zelTracerSetEnabled(zel_tracer_handle_t tracer, ze_bool_t enable,
                      const struct hpcrun_foil_appdispatch_level0* dispatch) {
