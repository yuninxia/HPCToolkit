...
CUpti_ActivityKind kernel_execution_activities[] = {
    ...
    // FIXME(keren): cupti does not allow the following activities to be profiled with
    // pc sampling CUPTI_ACTIVITY_KIND_GLOBAL_ACCESS, CUPTI_ACTIVITY_KIND_SHARED_ACCESS,
    // CUPTI_ACTIVITY_KIND_BRANCH,
    CUPTI_ACTIVITY_KIND_INVALID};

CUpti_ActivityKind overhead_activities[] = {CUPTI_ACTIVITY_KIND_OVERHEAD,
                                            CUPTI_ACTIVITY_KIND_INVALID};

CUpti_ActivityKind driver_activities[] = {CUPTI_ACTIVITY_KIND_DEVICE,
                                          CUPTI_ACTIVITY_KIND_DRIVER,
                                          CUPTI_ACTIVITY_KIND_INVALID};

CUpti_ActivityKind runtime_activities[] = {CUPTI_ACTIVITY_KIND_DEVICE,
...
static void METHOD_FN(thread_init) { TMSG(CUDA, "thread_init"); }

static void METHOD_FN(thread_init_action) { TMSG(CUDA, "thread_init_action"); }
static void METHOD_FN(start) {
  TMSG(CUDA, "start");
  TD_GET(ss_state)[self->sel_idx] = START;
}

static void METHOD_FN(thread_fini_action) { TMSG(CUDA, "thread_fini_action"); }
...
METHOD_FN(process_event_list)
{
  ...
  if (control_knob_value_get_int("HPCRUN_CUDA_DEVICE_BUFFER_SIZE",
                                 &device_buffer_size) != 0)
    auditor_exports()->exit(-1);

  // Note: no longer used since CUDA 12.3
  int device_semaphore_size;
  if (control_knob_value_get_int("HPCRUN_CUDA_DEVICE_SEMAPHORE_SIZE",
                                 &device_semaphore_size) != 0)
    auditor_exports()->exit(-1);

  char* kernel_serialization = NULL;
  if (control_knob_value_get_string("HPCRUN_CUDA_KERNEL_SERIALIZATION",
                                    &kernel_serialization) != 0)
    auditor_exports()->exit(-1);
...
static void METHOD_FN(finalize_event_list) { cupti_enable_activities(); }
...
static void METHOD_FN(display_events) {
  ...
  display_header(stdout, "Available events for monitoring CUDA on NVIDIA GPUs");

  display_header_event(stdout);

  display_event_info(stdout, NVIDIA_CUDA,
    "Comprehensive operation-level monitoring of CUDA on NVIDIA GPUs. "
