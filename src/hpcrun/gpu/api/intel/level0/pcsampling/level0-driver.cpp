...
{
  if (driver == nullptr) {
    pcsampling::error("Null driver handle passed to fetchDriverVersion");
  ...
  if (status != ZE_RESULT_SUCCESS) {
    pcsampling::error("Failed to get driver API version: %s", ze_result_to_string(status));
    // Set version to a known invalid value to indicate error condition
    version = ZE_API_VERSION_FORCE_UINT32;
  }
}
...
)
{
  ...
  if (printVersion) {
    pcsampling::warn("Level Zero API version: %u.%u", major, minor);
  }

  // Verify that the driver version is acceptable.
  if ((major < requiredMajor) || (major == requiredMajor && minor < requiredMinor)) {
    pcsampling::error("Level Zero API version %u.%u is lower than required %u.%u",
...
)
{
  ...
  if (drivers.empty()) {
    pcsampling::error("No Level Zero drivers available");
    // Set version to a known invalid value to indicate error condition
    version = ZE_API_VERSION_FORCE_UINT32;
    return;
  }
