...
reportWarning(const char* fmt, ...)
{
    if (hpcrun_api && hpcrun_api->warning_handler) {
        va_list args;
        va_start(args, fmt);
        hpcrun_api->warning_handler(fmt, args);
        va_end(args);
    }
}
...
int
computeHashString(const void* data, size_t size, char* out, unsigned int out_len)
{
    if (hpcrun_api && hpcrun_api->crypto_compute_hash_string) {
        return hpcrun_api->crypto_compute_hash_string(data, size, out, out_len);
    }
    return -1;
}
...
void
mcsLock(mcs_lock_t* lock, mcs_node_t* node)
{
    if (hpcrun_api && hpcrun_api->mcs_lock) {
        hpcrun_api->mcs_lock(lock, node);
    }
}


void
mcsUnlock(mcs_lock_t* lock, mcs_node_t* node)
{
    if (hpcrun_api && hpcrun_api->mcs_unlock) {
        hpcrun_api->mcs_unlock(lock, node);
    }
}
...
ze_result_t
callZeDeviceGetRootDevice(ze_device_handle_t hDevice,
                     ...
                     const struct hpcrun_foil_appdispatch_level0* dispatch)
{
    if (hpcrun_api && hpcrun_api->f_zeEventPoolCreate) {
        return hpcrun_api->f_zeEventPoolCreate(hContext, desc, numDevices, phDevices, phEventPool, dispatch);
    }
    return ZE_RESULT_ERROR_UNINITIALIZED;
}
...
ze_result_t
callZetContextActivateMetricGroups(zet_context_handle_t hContext, ze_device_handle_t hDevice,
                ...
                const struct hpcrun_foil_appdispatch_level0* dispatch)
{
    if (hpcrun_api && hpcrun_api->f_zetMetricGroupCalculateMultipleMetricValuesExp) {
        return hpcrun_api->f_zetMetricGroupCalculateMultipleMetricValuesExp(
            hMetricGroup, type, rawDataSize, pRawData, pSetCount,
            pTotalMetricValueCount, pMetricCounts, pMetricValues, dispatch);
    }
    return ZE_RESULT_ERROR_UNINITIALIZED;
}
...
lookupKernelSize(const char* kernel_name)
{
    ...
    return level0GetKernelSize(std::string(kernel_name));
}
