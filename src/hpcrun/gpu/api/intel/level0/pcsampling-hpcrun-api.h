...
typedef struct pcsampling_hpcrun_api_t {
    ...
    void (*register_cleanup_handler)(pcsampling_cleanup_handler_t handler, void* arg);
    ...
    // Kernel size lookup - properly accessible from libhpcrun
    size_t (*kernel_size_lookup)(const char* kernel_name);

    // Level Zero function pointers - the dispatch table for Level Zero calls
    // Core API functions (ze_*)
    ...
    ze_result_t (*f_zeDeviceGetProperties)(ze_device_handle_t hDevice,
                                           ...
                                           const struct hpcrun_foil_appdispatch_level0* dispatch);
    ze_result_t (*f_zeDeviceGetRootDevice)(ze_device_handle_t hDevice, ze_device_handle_t* phRootDevice,
                                           const struct hpcrun_foil_appdispatch_level0* dispatch);
    ...
    // Tools API functions (zet_*)
    ze_result_t (*f_zetContextActivateMetricGroups)(ze_context_handle_t hContext,
