...
Context::~Context() noexcept {
  ...
  }; // If we run into errors, just let the recursion handle it.
}
...
template <class Ctx>
using perctx_mvals_t =
    util::locked_unordered_map<util::reference_index<const Ctx>, mvals_t>;
...
void ContextFlowGraph::freeze(
  ...
  assert((m_templates.empty() || m_handler) &&
         "FlowGraph::Templates only make sense with a handler!");
  ...
  for (size_t i = 1; i < m_templates.size(); i++) {
    ...
    if (a.path().size() <= b.path().size()) {
      if (std::mismatch(a.path().rbegin(), a.path().rend(), b.path().rbegin()).first ==
          a.path().rend()) {
        util::log::fatal{}
            << "FlowGraph::Template paths cannot be suffixes of each other!";
      }
    }
  }
  ...
  if (m_siblings.count(scope()) > 0)
    util::log::fatal{}
        << "Recursive ContextFlowGraph was generated, unable to continue!";

  m_frozen_once.signal();
}

const std::deque<ContextFlowGraph::Template>&
ContextFlowGraph::templates() const noexcept {
  m_frozen_once.wait();
  return m_templates;
}
...
std::pair<std::unordered_map<util::reference_index<const ContextReconstruction>,
                             std::vector<double>>,
          std::vector<bool>>
ContextFlowGraph::exteriorFactors(
    const std::unordered_set<util::reference_index<const ContextReconstruction>>&
        reconsts,
          ...
          for (const auto& [m, v] : mvs->citerate()) {
            ...
            if (handling.exterior) {
              if (metricExterior && exteriorIsAlsoLogical == handling.exteriorLogical) {
                ...
                util::log::fatal{}
                    << "Multiple suitable Metrics for exterior factor calculations: "
...
std::vector<double>
ContextFlowGraph::interiorFactors(const perctx_mvals_t<ContextFlowGraph>& fg_data,
...
std::vector<double>
ContextFlowGraph::interiorFactors_impl(const Find& find, const At& at,
                                       const ForAll& forall,
                                       const std::vector<bool>& hasEC) const {
        ...
        forall(*mvs, [&](const Metric& m, double) {
          ...
          if (m_handler(m).interior) {
            if (metricInterior) {
              ...
              // factor calculations. We don't know which to pick, so abort.
              util::log::fatal{}
                  << "Multiple suitable Metrics for interior factor calculations: "
...
template <class T, class Find, class At, class ForAll>
std::pair<std::vector<double>, std::vector<bool>>
ContextReconstruction::rescalingFactors_impl(const Find& find, const At& at,
                                             const ForAll& forall) const {
  m_instantiated.wait();
    ...
    for (const auto& [entry_s, entry_c] : m_entries) {
      ...
      if (mvs) {
        ...
        if (!metricExterior && !exteriorLogicalIsAlsoExterior) {
          ...
          // recover in this case, so for now just abort.
          util::log::fatal{}
              << "No suitable Metrics for rescaling factor calculations!";
        }
