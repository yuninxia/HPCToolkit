...
namespace {
template <class I> constexpr I align(I v, std::uint8_t a) {
  return (v + a - 1) / a * a;
}
} // namespace

/// Engine that determines the final location for data blocks in the given
/// File::Instance.
...
public:
  ...
  /// Allocate the next available space of the given size and alignment, and return it's
  /// offset.
  ...
  std::uint_fast64_t size() const noexcept { return cursor; }
...
namespace {
template <class, class = void> struct is_constant_size : std::false_type {
...
template <class T>
struct is_array<T, std::void_t<decltype(std::declval<T>().elementOffset(42))>>
    : std::true_type {};

template <class, class = void> struct has_parent : std::false_type {};
template <class T>
struct has_parent<T, std::void_t<typename T::parent_type>> : std::true_type {};
} // namespace

/// Wrapper for a block of data written to a File::Instance. Knows its offset in the
/// file. Note that once the data is Written, it is considered constant for all intents
/// and purposes.
// MT: Externally Synchronized
template <class T, class Traits = DataTraits<T>> class Written {
public:
  Written(LayoutEngine& layout, T in_data = T(), Traits in_traits = Traits())
      : data(std::move(in_data)), traits(std::move(in_traits)) {
    typename is_constant_size<Traits>::serialized_type buf =
        traits.serialize((const T&)data);
    m_size = buf.size();
...
public:
  SubWritten(LayoutEngine& layout, typename Traits::parent_type& parent,
             T in_data = T(), Traits in_traits = Traits())
      : Written<T, Traits>(layout, std::move(in_data), std::move(in_traits)) {
    this->traits.update_parent(parent, this->ptr(), layout.size());
  }
};

/// RAII guard similar to #Written, except that it writes on the destructor instead of
/// the constructor. This allows the data to be altered before the actual write, but
/// makes it limited to only constant-size data.
// MT: Externally Synchronized
template <class T, class Traits = DataTraits<T>> class WriteGuard {
public:
...
public:
  SubWriteGuard(LayoutEngine& layout, typename Traits::parent_type& parent,
                T in_data = T())
...
template <class Traits> struct Array {
  Array(Traits et = Traits()) : element_traits(std::move(et)) {}
  ~Array() = default;

  static inline std::uint8_t alignment = Traits::alignment;
  template <class T> std::vector<char> serialize(const T& arr) {
    std::vector<char> buf;
    for (const auto& val : arr) {
      buf.resize(align(buf.size(), alignment), 0);
      typename is_constant_size<Traits>::serialized_type subbuf =
          element_traits.serialize(val);
...
template <class Traits> class DynamicArray {
public:
  ...
  template <class I> std::size_t elementOffset(I i) const { return suboffsets[i]; }
