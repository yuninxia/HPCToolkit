...
util::WorkshareResult SparseDB::help() {
  return forEachThread.contributeWhileAble() + forProfilesParse.contributeWhileAble() +
...
void SparseDB::notifyThreadFinal(const PerThreadTemporary& tt) {
  ...
  profDataOut.write(std::move(mvalsBuf), pi.valueBlock.pValues, std::move(cidxsBuf),
...
readProfileCtxPairs(const util::File& pmf, const fmt_profiledb_profInfo_t& pi) {
  ...
  prof_ctx_pairs.push_back(
      {std::numeric_limits<uint32_t>::max(), pi.valueBlock.nValues});
...
ProfileMetricData::ProfileMetricData(
  ...
  auto pmfi = pmf.open(false, false);
  pmfi.readat(offset + first->second * FMT_PROFILEDB_SZ_MVal, mvBlob.size(),
              mvBlob.data());
}

// Transpose and write the metric data for a range of contexts
static void writeContexts(uint32_t firstCtx, uint32_t lastCtx, const util::File& cmf,
  ...
  std::vector<std::pair<
      std::vector<std::pair<uint32_t, uint64_t>>::const_iterator, // ctx_id/idx pair in
                                                                  // a profile
  ...
  while (!heap.empty() && heap.front().first->first < lastCtx) {
    ...
    while (!heap.empty() && heap.front().first->first == ctx_id) {
      ...
      const char* cur = &profile.mvBlob[(curPair.second - profile.first->second) *
                                        FMT_PROFILEDB_SZ_MVal];
    ...
    const auto newsz =
        allpvs * FMT_CCTDB_SZ_PVal + valuebufs.size() * FMT_CCTDB_SZ_MIdx;
...
void SparseDB::write() {
    ...
    if (mpi::World::rank() == 0) {
      ...
      udc.nMetrics = std::accumulate(iter.begin(), iter.end(), (uint16_t)0,
                                     [](uint16_t out, const auto& mu) -> uint16_t {
                                       ...
                                       assert((mu.first->scopes() & use) == use &&
                                              "Inconsistent Metric value usage data!");
  ...
  stdshim::transform_exclusive_scan(ctxOffsets.begin(), ctxOffsets.end(),
                                    ctxOffsets.begin(), ctxStart, std::plus<>{},
  ...
  std::vector<uint32_t> ctxRanges;
  {
    ...
    const uint64_t limit =
        std::min<uint64_t>(1024ULL * 1024 * 1024 * 3,
  ...
  if (mpi::World::rank() == 0) {
    // Rank 0 is in charge of writing out the summary profile in profile.db
    {
      ...
      for (const Context& c : contexts) {
        ...
        std::vector<std::reference_wrapper<const std::pair<
            const util::reference_index<const Metric>, StatisticAccumulator>>>
            pairs;
        ...
        std::sort(pairs.begin(), pairs.end(),
                  [=, &src_x](const auto& a, const auto& b) {
                    return a.get().first->userdata[src_x.identifier()].base() <
                           b.get().first->userdata[src_x.identifier()].base();
                  });
        for (const auto& mx : pairs) {
    ...
    // in the cct.db
    {
      ...
      for (const Context& c : contexts) {
        ...
        cii.valueBlock.nValues = (ctxOffsets[i + 1] - ctxOffsets[i] -
                                  cii.valueBlock.nMetrics * FMT_CCTDB_SZ_MIdx) /
                                 FMT_CCTDB_SZ_PVal;
        cii.valueBlock.pValues = ctxOffsets[i];
        cii.valueBlock.pMetricIndices =
            cii.valueBlock.pValues + cii.valueBlock.nValues * FMT_CCTDB_SZ_PVal;
  ...
  while (idx < ctxRanges.size() - 1) {
    ...
    if (firstCtx < lastCtx) {
      ...
      forProfilesLoad.fill(metricData.size(), [this, &metricData, firstCtx, lastCtx,
                                               &profiles](size_t i) {
      ...
      std::vector<std::pair<uint32_t, uint32_t>> ctxRanges;
      {
        ...
        const size_t target =
            (ctxOffsets[lastCtx] - ctxOffsets[firstCtx]) / src.teamSize();
        size_t cursize = 0;
        for (uint32_t id = firstCtx; id < lastCtx; id++) {
          ...
          if (cursize > target) {
            ctxRanges.push_back(
                {!ctxRanges.empty() ? ctxRanges.back().second : firstCtx, id + 1});
            cursize = 0;
          }
        }
        if (ctxRanges.empty() || ctxRanges.back().second < lastCtx) {
          ...
          ctxRanges.push_back(
              {!ctxRanges.empty() ? ctxRanges.back().second : firstCtx, lastCtx});
        }
      }
  ...
  if (mpi::World::rank() + 1 == mpi::World::size()) {
    cmf->open(true, false)
        .writeat(ctxOffsets.back(), sizeof fmt_cctdb_footer, fmt_cctdb_footer);
  }
}
