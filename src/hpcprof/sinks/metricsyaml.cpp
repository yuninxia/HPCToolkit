...
static std::string anchorName(const Metric& m, const StatisticPartial& p,
                              MetricScope s) {
  std::ostringstream ss;
...
static std::string anchorName(const Metric& m, MetricScope s) {
  std::ostringstream ss;
  ss << m.name() << '-' << Statistic::combination_t::sum << '-'
     << Expression(Expression::variable) << '-' << s;
  return sanitize(ss.str());
}
...
static void
claim(std::vector<std::reference_wrapper<const Metric>>& into,
...
static bool rawFormula(YAML::Emitter& out, const Metric& m, const Statistic& s,
                       MetricScope ms, const std::string& key) {
...
static bool rawFormula(YAML::Emitter& out, const ExtraStatistic& es, MetricScope ms,
                       const std::string& key) {
  using namespace YAML;
  if (!es.scopes().has(ms))
    return false;
  out << Key << key << Value << Flow;
  es.formula().citerate_all([&](double v) { out << v; },
                            [&](Expression::uservalue_t v) {
                              assert(v != 0);
...
static void rawLeafVariants(YAML::Emitter& out, const Metric& m) {
  ...
  for (const auto& s : m.statistics()) {
    ...
    if (m.scopes().has(MetricScope::lex_aware) ||
        m.scopes().has(MetricScope::function) || m.scopes().has(MetricScope::point)) {
...
static void rawLeafVariants(YAML::Emitter& out, const ExtraStatistic& es) {
  using namespace YAML;
  out << Key << "variants" << Value << BeginMap << Key << "" << Value << BeginMap << Key
  ...
  if (es.scopes().has(MetricScope::lex_aware) ||
      es.scopes().has(MetricScope::function) || es.scopes().has(MetricScope::point)) {
    out << Key << "exclusive" << Value << BeginMap;
...
static void rawLeaf(YAML::Emitter& out, const ExtraStatistic& es) {
  using namespace YAML;
  out << BeginMap << Key << "name" << Value << es.name() << Key << "description"
...
struct GPU {
  struct Ker {
    ...
    void dump(YAML::Emitter& out) {
      ...
      if (!time)
        return;
      out << BeginMap << Key << "name" << Value << "Kernel Execution" << Key
          << "description" << Value << "Time spent running kernels on a GPU.";
  ...
  struct Sync {
    ...
    void dump(YAML::Emitter& out) {
      ...
      if (kinds.empty())
        return;
      out << BeginMap << Key << "name" << Value << "Synchronization" << Key
          << "description" << Value
          << "Time spent idle waiting for actions from the host / other GPU operations."
          << Key << "variants" << Value << BeginMap << Key << "Sum" << Value << BeginMap
          << Key << "render" << Value << Flow << BeginSeq << "number" << "percent"
          << "colorbar" << EndSeq << Key << "formula" << Value << "sum" << EndMap
          << EndMap << Key << "children" << Value << BeginSeq;
  ...
  struct MemCpy {
    ...
    void dump(YAML::Emitter& out) {
      ...
      if (!time_x && !time_i)
        return;
      out << BeginMap << Key << "name" << Value << "Memory Transfer" << Key
          << "description" << Value
          << "Time spent in data transfers to, from and within GPU memory" << Key
          << "variants" << Value << BeginMap << Key << "Sum" << Value << BeginMap << Key
          << "render" << Value << Flow << BeginSeq << "number" << "percent"
          << "colorbar" << EndSeq << Key << "formula" << Value << "sum" << EndMap
          << EndMap << Key << "children" << BeginSeq;
      if (time_x) {
        out << BeginMap << Key << "name" << Value << "Explicit Memory Transfer" << Key
            << "description"
            << "Time spent in data transfers explicitly performed by the user" << Key
            << "variants" << Value << BeginMap;
        for (const auto& s : time_x->statistics()) {
          out << Key << s.suffix() << Value << BeginMap << Key << "render" << Value
  ...
  struct MemSet {
    ...
    void dump(YAML::Emitter& out) {
      ...
      if (!time)
        return;
      out << BeginMap << Key << "name" << Value << "Memory Batch Clear" << Key
          << "description" << Value
          << "Operations that set large regions of GPU-related memory" << Key
          << "variants" << Value << BeginMap;
      ...
      out << EndMap;
      if (!sub.empty()) {
        out << Key << "children" << Value << BeginSeq << BeginMap << Key << "name"
            << Value << "Bytes Cleared" << Key << "description" << Value
            << "Total number of bytes cleared" << Key << "variants" << Value << BeginMap
            << Key << "Sum" << Value << BeginMap << Key << "render" << Value << Flow
            << BeginSeq << "number" << "percent" << "colorbar" << EndSeq << Key
            << "formula" << Value << "sum" << EndMap << EndMap << Key << "children"
            << Value << BeginSeq;
  ...
  struct MemAlloc {
    ...
    void dump(YAML::Emitter& out) {
      ...
      if (!time)
        return;
      out << BeginMap << Key << "name" << Value << "Memory De/Allocation" << Key
          << "description" << Value
          << "Operations that allocate or deallocate GPU-related memory" << Key
          << "variants" << Value << BeginMap;
      for (const auto& s : time->statistics()) {
        out << Key << s.suffix() << Value << BeginMap << Key << "render" << Value
            << Flow << BeginSeq << "number" << "hidden";
      ...
      if (!sub.empty()) {
        out << Key << "children" << Value << BeginSeq << BeginMap << Key << "name"
            << Value << "Bytes Allocated/Freed" << Key << "description" << Value
            << "Total number of bytes allocated/freed" << Key << "variants" << Value
            << BeginMap << Key << "Sum" << Value << BeginMap << Key << "render" << Value
            << Flow << BeginSeq << "number" << "percent" << "colorbar" << EndSeq << Key
            << "formula" << Value << "sum" << EndMap << EndMap << Key << "children"
            << Value << BeginSeq;
        for (const Metric& sm : sub)
          rawLeaf(out, sm);
  ...
  void dump(YAML::Emitter& out) {
    ...
    out << BeginMap << Key << "name" << Value << "GPU" << Key << "description" << Value
        << "GPU-accelerated performance metrics" << Key << "variants" << Value
        << BeginMap << Key << "Sum" << Value << BeginMap << Key << "render" << Value
        << Flow << BeginSeq << "number" << "percent" << "colorbar" << EndSeq << Key
        << "formula" << Value << "sum" << EndMap << EndMap << Key << "children" << Value
        << BeginSeq;
    c_ker.dump(out);
    c_sync.dump(out);
    c_memcpy.dump(out);
...
struct CPU {
  ...
  void dump(YAML::Emitter& out) {
    ...
    out << BeginMap << Key << "name" << Value << "CPU" << Key << "description" << Value
        << "Time spent within the CPU" << Key << "variants" << Value << BeginMap << Key
        << "Sum" << Value << BeginMap;
    if (realtime || cputime) {
      out << Key << "render" << Value << Flow << BeginSeq << "number" << "percent"
          << EndSeq << Key << "formula" << Value << "first";
    } else {
...
void MetricsYAML::standard(std::ostream& os) {
  ...
  std::vector<std::variant<std::reference_wrapper<const Metric>,
                           std::reference_wrapper<const ExtraStatistic>>>
      mets;
  ...
  for (const Metric& m : src.metrics().citerate()) {
    ...
    const auto f = [&](MetricScope s) {
      ...
      for (const auto& p : m.partials()) {
        out << Anchor(anchorName(m, p, s)) << BeginMap << Key << "metric" << Value
            << m.name() << Key << "scope" << Value << s << Key << "formula" << Value
            << MetaDB::accumulateFormulaString(p.accumulate()) << Key << "combine"
            << Value << p.combinator() << EndMap;
      }
      return true;
    };
  ...
  for (const auto& x : mets) {
    std::visit([&](auto ref_x) { rawLeaf(out, ref_x); }, x);
  }
  out << EndSeq;
