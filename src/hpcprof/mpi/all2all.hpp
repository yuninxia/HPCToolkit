...
void gatherv_root(void* data, const std::size_t* cnts, const Datatype&,
                  std::size_t rootRank);
...
void scatterv_root(void* data, const std::size_t* cnts, const Datatype&,
                   std::size_t rootRank);
...
std::optional<std::vector<std::array<T, N>>> gather(std::array<T, N> data,
                                                    std::size_t root) {
  if (World::rank() == root) {
...
template <class T>
std::optional<std::vector<std::vector<T>>> gather(std::vector<T> data,
                                                  std::size_t root) {
  if (World::rank() == root) {
    ...
    detail::gatherv_root(buffer.data(), cnts.value().data(), detail::asDatatype<T>(),
                         root);
...
std::optional<std::vector<std::basic_string<C, T>>> gather(std::basic_string<C, T> data,
                                                           std::size_t root) {
...
template <class C, class T>
std::basic_string<C, T> scatter(std::vector<std::basic_string<C, T>> data,
                                std::size_t root) {
  assert(data.size() == World::size() && "Invalid data argument to mpi::scatter!");
...
gather(std::vector<std::basic_string<C, T>> data, std::size_t root) {
  if (World::rank() == root) {
    ...
    for (std::size_t r = 0; r < World::size(); r++) {
      ...
      for (std::size_t i = 0, idx = 0; i < lengths[r].size();
           idx += lengths[r][i], i++) {
        if (lengths[r][i] > 0) {
