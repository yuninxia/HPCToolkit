...
double Expression::constant() const {
  assert(m_kind == Kind::constant &&
         "Attempt to get the constant value of a non-constant Expression");
  return std::get<double>(m_data);
}
const Expression& Expression::subexpr() const {
  assert(m_kind == Kind::subexpression &&
...
Expression::uservalue_t Expression::var() const {
  assert(m_kind == Kind::variable &&
         "Attempt to get the user-value of a non-variable Expression");
  return std::get<uservalue_t>(m_data);
}
const std::vector<Expression>& Expression::op_args() const {
  assert(m_kind >= Kind::first_op &&
         "Attempt to get the arguments of a non-operation Expression");
  return std::get<std::vector<Expression>>(m_data);
}

void Expression::optimize() { optimize_impl(nullptr); }
