...
void Function::sourceLocation(const File& file, uint64_t line) noexcept {
  assert(!m_file &&
         "Attempt to overwrite a Function's source location, use += instead!");
  m_file = file;
  m_line = line;
}
...
void Function::mergeSmall(const Function& o) noexcept {
  if (!m_offset || (o.m_offset && *m_offset > *o.m_offset))
    m_offset = o.m_offset;
  if (!m_file ||
      (o.m_file && (m_file->path() > o.m_file->path() ||
                    (m_file == o.m_file &&
                     (m_line == 0 || (o.m_line > 0 && m_line > o.m_line)))))) {
    m_file = o.m_file;
    m_line = o.m_line;
  }
}
