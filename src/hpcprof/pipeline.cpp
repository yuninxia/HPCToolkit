...
size_t ProfilePipeline::TupleHash::operator()(
    const std::vector<pms_id_t>& tuple) const noexcept {
  size_t sponge = 0x15;
...
detail::ProfilePipelineBase::SourceEntry::SourceEntry(
    std::unique_ptr<ProfileSource>&& up)
...
Settings& Settings::operator<<(ProfileSink& s) {
  ...
  requested += req;
  assert(req - available == ExtensionClass() &&
         "Sink requires unavailable extended data!");
...
Settings& Settings::operator<<(ProfileFinalizer& f) {
  ...
  assert(req - available == ExtensionClass() &&
         "Finalizer requires unavailable extended data!");
  requested += req;
  available += pro;
...
ProfilePipeline::ProfilePipeline(Settings&& b, std::size_t team_sz)
  ...
  cct.reset(
      new Context(structs.context, std::nullopt, {Relation::global, Scope(*this)}));
  for (auto& s : sinks) {
  ...
  for (auto& ms : sources) {
    ...
    ms().bindPipeline(
        Source(*this, ms.dataLimit, ExtensionClass::all(), sourceLocals[idx]));
...
void ProfilePipeline::complete(
  ...
  auto drain = [&](auto& tpd, auto type, auto notify) {
    ...
    if (tt.minTime > std::chrono::nanoseconds::min()) {
      if (localTimepointBounds) {
        ...
        localTimepointBounds->second =
            std::max(localTimepointBounds->second, tt.maxTime);
      } else
        localTimepointBounds = {tt.minTime, tt.maxTime};
    }
  };
  drain(tt.ctxTpData, DataClass::ctxTimepoints,
  ...
  tt.finalize();
  std::shared_ptr<PerThreadTemporary> ttptr =
      std::make_shared<PerThreadTemporary>(std::move(tt));
  for (auto& s : sinks)
    if (s.dataLimit.hasThreads())
      s().notifyThreadFinal(ttptr);
}
...
#pragma omp for schedule(dynamic) nowait
    ...
    if (localTimepointBounds) {
      ...
      if (timepointBounds) {
        timepointBounds->first =
            std::min(timepointBounds->first, localTimepointBounds->first);
        timepointBounds->second =
            std::max(timepointBounds->second, localTimepointBounds->second);
      } else
...
#pragma omp for schedule(dynamic) nowait
    ...
    std::forward_list<std::reference_wrapper<SinkEntry>> workingSinks(sinks.begin(),
                                                                      sinks.end());
    bool didwork = true;
    do {
...
const decltype(ProfilePipeline::Extensions::identifier)& Source::identifier() const {
  assert(extensionLimit.hasIdentifier() &&
         "Source did not register for `identifier` emission!");
  return pipe->uds.identifier;
}
const decltype(ProfilePipeline::Extensions::resolvedPath)&
Source::resolvedPath() const {
  assert(extensionLimit.hasResolvedPath() &&
         "Source did not register for `resolvedPath` emission!");
  return pipe->uds.resolvedPath;
}

#define SRC_ASSERT_LIMITS(DS)                                                          \
  do {                                                                                 \
    assert(dataLimit.has(DataClass::DS) && "Source did not register for `" #DS         \
                                           "` emission!");                             \
    assert(pipe->scheduled.has(DataClass::DS) &&                                       \
           "`" #DS "` are not scheduled for this Pipeline!");                          \
    if (slocal != nullptr)                                                             \
      assert(!slocal->disabled.has(DataClass::DS) &&                                   \
             "Attempt to emit `" #DS "` after Source has already read(`" #DS "`)!");   \
  } while (0)

#define SRC_ASSERT_LIMITS_MULTI(DS)                                                    \
  do {                                                                                 \
    assert(dataLimit.anyOf(DS) && "Source did not register for `" #DS "` emission!");  \
    assert(pipe->scheduled.anyOf(DS) && "`" #DS                                        \
                                        "` are not scheduled for this Pipeline!");     \
    if (slocal != nullptr)                                                             \
      assert(!slocal->disabled.allOf(DS) &&                                            \
             "Attempt to emit `" #DS "` after Source has already read(`" #DS "`)!");   \
  } while (0)
...
void Source::timepointBounds(std::chrono::nanoseconds min,
...
Module& Source::module(const stdshim::filesystem::path& p,
                       const stdshim::filesystem::path& rp) {
  SRC_ASSERT_LIMITS(references);
...
void Source::addToReconstructionGroup(ContextFlowGraph& g, PerThreadTemporary& t,
                                      uint64_t gid) {
...
void Source::addToReconstructionGroup(Context& r, const Scope& entry,
                                      PerThreadTemporary& t, uint64_t gid) {
  SRC_ASSERT_LIMITS(contexts);
  auto& group = t.r_groups[gid];
  std::unique_lock<std::mutex> l(group.lock);
...
Source::AccumulatorsRef Source::accumulateTo(PerThreadTemporary& t,
                                             ContextReconstruction& cr) {
  SRC_ASSERT_LIMITS(metrics);
  assert(slocal->lastWave && "Attempt to emit metrics before requested!");
  return AccumulatorsRef(t.r_data[cr]);
}

Source::AccumulatorsRef Source::accumulateTo(PerThreadTemporary& t, uint64_t g,
                                             Context& c) {
  SRC_ASSERT_LIMITS(metrics);
  assert(slocal->lastWave && "Attempt to emit metrics before requested!");
...
void Source::AccumulatorsRef::add(Metric& m, double v) { map.get()[m].add(v); }
...
PerThreadTemporary& Source::mergedThread(ThreadAttributes o) {
  ...
  auto& t = newThread(std::move(o));
  auto [x_tt, first] =
      pipe->mergedThreads.emplace(t.attributes.idTuple(), PerThreadTemporary(t));
  assert(first);
...
Source::timepoint(PerThreadTemporary& tt, PerThreadTemporary::TimepointsData<Tp>& tpd,
...
Source::TimepointStatus Source::timepoint(PerThreadTemporary& tt, Context& c,
...
Source::TimepointStatus Source::timepoint(PerThreadTemporary& tt, Metric& m, double v,
                                          std::chrono::nanoseconds tm) {
  SRC_ASSERT_LIMITS(ctxTimepoints);
  assert(slocal->lastWave && "Attempt to emit timepoints before requested!");
...
Sink::Sink(ProfilePipeline& p, const DataClass& d, const ExtensionClass& e,
           std::size_t i)
    : pipe(&p), dataLimit(d), extensionLimit(e), idx(i), orderedWavefront(false),
      priorWavefrontDepOnce(std::numeric_limits<std::size_t>::max()),
      orderedWrite(false), priorWriteDepOnce(std::numeric_limits<std::size_t>::max()){};

void Sink::registerOrderedWavefront() {
  assert(!pipe->depChainComplete && "Attempt to register a Sink for an ordered "
                                    "wavefront chain after notifyPipeline!");
  if (!orderedWavefront) {
...
util::Once::Caller Sink::enterOrderedWavefront() {
  assert(orderedWavefront &&
         "Attempt to enter an ordered wavefront region without registering!");
...
util::Once::Caller Sink::enterOrderedWrite() {
  assert(orderedWrite &&
         "Attempt to enter an ordered write region without registering!");
...
const decltype(ProfilePipeline::Extensions::identifier)& Sink::identifier() const {
  assert(extensionLimit.hasIdentifier() &&
         "Sink did not register for `identifier` absorption!");
  return pipe->uds.identifier;
}
const decltype(ProfilePipeline::Extensions::resolvedPath)& Sink::resolvedPath() const {
  assert(extensionLimit.hasResolvedPath() &&
         "Sink did not register for `resolvedPath` absorption!");
  return pipe->uds.resolvedPath;
}

const ProfileAttributes& Sink::attributes() {
  assert(dataLimit.hasAttributes() &&
...
const util::locked_unordered_uniqued_set<Module, stdshim::shared_mutex,
                                         util::uniqued_hash<stdshim::hash_path>>&
Sink::modules() {
  assert(dataLimit.hasReferences() &&
         "Sink did not register for `references` absorption!");
  return pipe->mods;
}
...
std::size_t Sink::teamSize() const noexcept { return pipe->team_size; }
