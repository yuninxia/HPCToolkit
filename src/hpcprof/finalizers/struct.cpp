...
public:
  ...
  std::optional<LMData>
  seekToNextLM(const stdshim::filesystem::path measDirPath) noexcept;
  bool parse(ProfilePipeline::Source&, const Module&, bool,
             StructFile::udModule&) noexcept;
...
StructFile::StructFile(stdshim::filesystem::path p, stdshim::filesystem::path meas,
                       std::shared_ptr<RecommendationStore> rs)
    : recstore(std::move(rs)), path(stdshim::filesystem::absolute(std::move(p))),
      measDirPath(stdshim::filesystem::weakly_canonical(meas)) {
    ...
    do {
      if (auto o_lm = parser->seekToNextLM(measDirPath)) {
        *lm = std::move(*o_lm);
        ...
        // EOF or error
        if (!parser->valid())
          util::log::warning{} << "Failed to parse Structfile "
...
bool StructFile::resolve(ContextFlowGraph& fg) noexcept {
  if (fg.scope().type() == Scope::Type::point) {
    ...
    case CallGraphStatus::ERRORED:
      ...
      util::call_once(recstore->once, [&] {
        util::log::warning() << "Control flow data is corrupt, disabling affected GPU "
    ...
    const std::function<void(const Function&)> dfs = [&](const Function& callee) {
      // Step to any potential callers. If this function can be called, this is part of
      // the path.
      bool terminal = true;
      for (auto [callerit, end] = udm.rcg.equal_range(callee); callerit != end;
           ++callerit) {
        terminal = false;
...
void StructFile::load(const Module& m, udModule& ud) noexcept {
  ...
  ud.cfgStatus =
      lm->has_calls ? CallGraphStatus::ERRORED : CallGraphStatus::NOT_PRESENT;
...
std::optional<LMData>
StructFileParser::seekToNextLM(const stdshim::filesystem::path measDirPath) noexcept
    try {
  assert(ok);
  ok = false;
  LMData lm;
        ...
        if (ename == "LM") {
          ...
          if (!lm.path.has_root_path()) {
            if (measDirPath.empty()) {
              util::log::warning{} << "No measurement directory path provided for a "
                                      "load module with relative path "
...
bool StructFileParser::parse(ProfilePipeline::Source& sink, const Module& m,
  ...
  std::deque<
      std::pair<uint64_t, std::pair<uint64_t, std::reference_wrapper<const Function>>>>
      tmp_rcg;
  ...
  LHandler handler(
      [&](const std::string& ename, const Attributes& attr) {
        ...
        if (ename == "LM") { // Load Module
          ...
          ud.trie.push_back(
              {{Scope(Scope::loop, m, addr, file, line), Relation::enclosure},
               top.node});
          next.node = &ud.trie.back();
          next.file = file;
          ...
          if (ename == "C") { // Call: <S> with an additional call edge
            ...
            if (!callee.empty())
              tmp_rcg.push_back(
                  {(uint64_t)std::stoll(callee, nullptr, 16), {callerInst, *top.func}});
          }
        } else if (ename == "A") {
          if (top.tag != 'A') { // First A, gives the caller line.
            ...
            auto& func = ud.funcs.emplace_back(
                m, std::nullopt, xmlstr(attr.getValue(XMLStr("n"))), file,
                std::stoll(xmlstr(attr.getValue(XMLStr("l")))));
            auto& next = stack.emplace(top, 'B');
            next.file = file;
            ud.trie.push_back(
                {{Scope(*top.file, top.a_line), Relation::inlined_call}, top.node});
  ...
  // If we have a call graph, do some processing on it to generate a final Reverse Call
  // Graph (RCG).
  if (has_calls || !tmp_rcg.empty()) {
    ...
    for (const auto& [callee, caller] : tmp_rcg) {
      ...
      if (target_it != funcs.end()) {
        ...
        // One of the <C> tags is obviously not correct. This usually indicates an error
        // in hpcstruct's mapping of calls to procedures, so
    ...
    if (ok) {
      // At this point, the RCG may contain cycles from recursive functions. This makes
      // things harder later when we try to associate call paths, so we adjust the edges
      // until we achieve a proper DAG.

      // First, using Tarjan's algorithm to group all the Functions in the RCG together
      ...
      std::unordered_multimap<util::reference_index<const Function>,
          ...
          sccs;
      std::unordered_map<util::reference_index<const Function>,
                         std::reference_wrapper<const Function>>
          scc_roots;
      {
        struct FuncState {
        ...
        const std::function<std::pair<const FuncState&, bool>(const Function&)> scc =
          ...
          if (first) {
            ...
            for (auto [it, end] = ud.rcg.equal_range(func); it != end; ++it) {
              ...
              if (callerFirstVisit) {
                state.rootEncounterIndex =
                    std::min(state.rootEncounterIndex, callerState.rootEncounterIndex);
              } else if (callerState.isActive) {
                state.rootEncounterIndex =
                    std::min(state.rootEncounterIndex, callerState.encounterIndex);
              }
            }
      ...
      // Delete any edges where the callee and caller are in the same SCC. These edges
      // cause cycles which we want to remove. After this, the graph will be a DAG.
      ...
      // The previous step may have broken call paths that required some SCC-internal
      // edges. Recover the reachability properties of the original graph (between
      // Functions not in the same SCC) by "summarizing" the intra-SCC edges with
      // inter-SCC edges. In short, we pretend every call "could" call all other
      // Functions in the callee's SCC.
