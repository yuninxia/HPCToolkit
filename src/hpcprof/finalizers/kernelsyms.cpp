...
void KernelSymbols::load(const Module& m, udModule& ud) noexcept {
  ...
  if (m.relative_path().empty() ||
      m.relative_path().parent_path().filename() != "kernel_symbols")
    return;
  ...
  try {
    ...
    while (std::getline(symsfile, linestr)) {
      ...
      if (!line || typestr.size() != 1) {
        if (!sawBadLine) {
          sawBadLine = true;
          util::log::error{} << "Failed to parse entry from symbols file "
                             << syms.string() << ", some functions within " << name
    ...
    if (symsfile.bad()) {
      util::log::error{} << "I/O failure while reading from symbols file "
                         << syms.string();
    }
    ...
    util::log::vwarning{} << "Exception caught while parsing symbols data from " << syms
                          << " for " << m.path()
                          << "\n"
                             "  what(): "
                          << e.what();
