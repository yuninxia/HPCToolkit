...
static stdshim::filesystem::path altfile(const stdshim::filesystem::path& path,
                                         Elf* elf) {
  std::error_code ec;
      ...
      if (altfd != -1) {
        ...
        if (altdbg != nullptr) {
          if (dwarfThreshold == std::numeric_limits<uintmax_t>::max() ||
            ...
            util::log::warning{}
                << "Skipping DWARF for " << mpath.string() << ", over threshold ("
...
bool DirectClassification::symtab(void* elf_vp, const Module& m, udModule& ud) {
  ...
  case ELFCLASS32:
    return parse<Elf32_Shdr, Elf32_Sym, elf32_getshdr>(elf, m, f);
  case ELFCLASS64:
    return parse<Elf64_Shdr, Elf64_Sym, elf64_getshdr>(elf, m, f);
  default:
    return false;
  }
}
...
bool DirectClassification::fullDwarf(void* dbg_vp, const Module& m, udModule& ud) try {
  ...
  auto getFileDie = [&](Dwarf_Die* cudie, size_t idx,
                        bool allowUnknown) -> const File* {
    Dwarf_Files* files;
    size_t nfiles;
  ...
  while (dwarf_get_units(dbg, cu, &cu, nullptr, nullptr, &root, nullptr) == 0) {
    dwarfwalk<udModule::trienode*>(
        ...
        [&](Dwarf_Die& die, udModule::trienode* par) -> udModule::trienode* {
          ...
          Dwarf_Word idx = 0;
          if (dwarf_formudata(dwarf_attr_integrate(&die, DW_AT_decl_file, &attr_mem),
                              &idx) == 0 &&
              idx > 0) {
            Dwarf_Die cu_mem;
            auto* file =
                getFileDie(dwarf_diecu(&die, &cu_mem, nullptr, nullptr), idx, false);
            if (file != nullptr) {
              int linenum;
          ...
          {
            Dwarf_Word idx = 0;
            if (dwarf_formudata(dwarf_attr_integrate(&die, DW_AT_call_file, &attr_mem),
                                &idx) == 0 &&
                idx > 0) {
            ...
            for (auto [it, end] = ud.leaves.equal_range(mine); it != end; ++it) {
              ...
              if (!before.empty()) {
                [[maybe_unused]] bool first =
                    ud.leaves.try_emplace(before, *here).second;
                assert(first);
              }
              mine = after;
              if (mine.empty())
  ...
  util::log::info{} << "Exception caught during DWARF parsing for "
                    ...
                    << "\n"
                       "  Full path: "
                    << m.userdata[sink.resolvedPath()].string();
  return false;
}
