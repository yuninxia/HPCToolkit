...
LogicalFile::classify(Context& parent, NestedScope& ns) noexcept {
  if (ns.flat().type() == Scope::Type::point) {
    ...
    if (udm.isLogical) {
      ...
      if (const auto* prf =
              std::get_if<std::reference_wrapper<const File>>(&it->second)) {
        // Indicates a line within a File
        ns.flat() = {prf->get(), lineno};
        return {std::make_pair(std::nullopt, std::ref(parent))};
      }
      if (const auto* prf =
              std::get_if<std::reference_wrapper<const Function>>(&it->second)) {
...
void LogicalFile::load(const Module& m, udModule& data) noexcept {
  ...
  // TODO: Should we record an identifier in the file itself instead of using the
  // filename?
  ...
  do {
    ...
    char* filename = NULL;
    uint32_t lineno = 0;
    if ((ret = hpcfmt_int4_fread(&fid, f)) != HPCFMT_OK)
      break;
    if ((ret = hpcfmt_str_fread(&funcname, f, std::malloc)) != HPCFMT_OK) {
      ret = HPCFMT_ERR;
      break;
    }
    if (fread(&funcmang, sizeof(char), 1, f) < 1) {
      ret = HPCFMT_ERR;
      break;
    }
    if ((ret = hpcfmt_str_fread(&filename, f, std::malloc)) != HPCFMT_OK) {
      ret = HPCFMT_ERR;
      break;
    }
    if ((ret = hpcfmt_int4_fread(&lineno, f)) != HPCFMT_OK) {
      ret = HPCFMT_ERR;
      break;
    }
    ...
    default:
      util::log::fatal{} << "Invalid logical stanza, unrecognized mangling "
                         << funcmang;
    }
  ...
  if (ret != HPCFMT_EOF)
    util::log::warning{}
        << "Logical metadata file " << mpath
        << " appears to be truncated, some logical frames may be missing!";
