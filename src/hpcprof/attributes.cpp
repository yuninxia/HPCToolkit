...
void ThreadAttributes::ctxTimepointStats(unsigned long long cnt,
                                         unsigned int disorder) noexcept {
  assert(!m_ctxTimepointStats &&
         "Attempt to overwrite previously set timepoint stats!");
  m_ctxTimepointStats = {cnt, disorder};
}

unsigned long long
ThreadAttributes::metricTimepointMaxCount(const Metric& m) const noexcept {
...
void ThreadAttributes::metricTimepointStats(const Metric& m, unsigned long long cnt,
                                            unsigned int disorder) {
  [[maybe_unused]] bool newElem =
      m_metricTimepointStats.insert({m, {cnt, disorder}}).second;
...
void ThreadAttributes::idTuple(std::vector<pms_id_t> tuple) {
  assert(m_idTuple.empty() &&
         "Attempt to overwrite a previously set thread hierarchical id tuple!");
...
ThreadAttributes::FinalizeState::FinalizeState() {
  if (mpi::World::size() > 1 && mpi::World::rank() == 0) {
    server = std::thread([this] {
      while (auto query =
                 mpi::receive_server<std::uint16_t>(mpi::Tag::ThreadAttributes_1)) {
        auto [kind, src] = *query;
        auto& cmap = kind != IDTUPLE_INVALID
                         ? globalIdxMap[kind]
                         : localIdxMap[mpi::receive_vector<std::uint16_t>(
                               src, mpi::Tag::ThreadAttributes_1)];
        mpi::send<std::uint64_t>(
            cmap.get(mpi::receive<std::uint64_t>(src, mpi::Tag::ThreadAttributes_1)),
            src, mpi::Tag::ThreadAttributes_1);
      }
    });
  }
...
std::ostream& std::operator<<(std::ostream& os, const ThreadAttributes& ta) noexcept {
  ...
  for (const auto& kv : ta.idTuple()) {
    ...
    case IDTUPLE_THREAD:
      os << "THREAD(" << intr << "){";
      break;
    case IDTUPLE_GPUDEVICE:
      os << "GPUDEVICE(" << intr << "){";
      break;
    case IDTUPLE_GPUCONTEXT:
      os << "GPUCONTEXT(" << intr << "){";
      break;
    case IDTUPLE_GPUSTREAM:
      os << "GPUSTREAM(" << intr << "){";
      break;
    case IDTUPLE_CORE:
      os << "CORE(" << intr << "){";
      break;
    default:
      os << "[" << IDTUPLE_GET_KIND(kv.kind) << "](" << intr << "){";
      break;
    }
...
bool ProfileAttributes::merge(const ProfileAttributes& o) {
  ...
  if (!m_name)
    ...
    util::log::vwarning() << "Merging profiles with different names: `" << *m_name
                          << "' and `" << *o.m_name << "'!";
    ok = false;
  }
  ...
  if (!m_job)
    ...
    util::log::vwarning() << "Merging profiles with different job ids: " << *m_job
                          << " and " << *o.m_job << "!";
    ok = false;
  }
  ...
  for (const auto& e : o.m_env) {
    ...
    if (it == m_env.end())
      ...
      util::log::vwarning()
          << "Merging profiles with different values for environment `" << e.first
