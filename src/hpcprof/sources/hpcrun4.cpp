...
namespace {
template <class F> class scope_exit final {
public:
...
template <class F> scope_exit<std::decay_t<F>> make_scope_exit(F&& f) {
...
Hpcrun4::Hpcrun4(const stdshim::filesystem::path& fn,
    ...
    : ProfileSource(), fileValid(true), attrsValid(true), tattrsValid(true),
      thread(nullptr), path(fn), measDirPath(fs::weakly_canonical(meas)),
      tracepath(fn) {
  ...
  for (uint32_t i = 0; i < hdr.nvps.len; i++) {
    ...
    if (k == HPCRUN_FMT_NV_prog)
      ...
      util::log::vwarning() << "Unknown file attribute in " << path.string()
                            << ":\n"
                               "  '"
                            << k << "'='" << std::string(v) << "'!";
    }
...
bool Hpcrun4::setupTrace(unsigned int traceDisorder) noexcept {
  ...
  callTrace =
      HPCTRACE_HDR_FLAGS_GET_BIT(thdr.flags, HPCTRACE_HDR_FLAGS_CALL_TRACE_BIT_POS);
...
bool Hpcrun4::realread(const DataClass& needed) try {
  ...
  if (needed.hasAttributes()) {
    ...
    while ((id = hpcrun_sparse_next_metric(file, &m, 4.0)) > 0) {
      ...
      settings.orderId = id;
      settings.scopes = {MetricScope::execution, MetricScope::lex_aware,
                         MetricScope::function, MetricScope::point};
      switch (m.flags.fields.show) {
      case HPCRUN_FMT_METRIC_SHOW:
        break; // Default
      ...
      case HPCRUN_FMT_METRIC_SHOW_INCLUSIVE:
        util::log::verror{}
            << "Show parameter SHOW_INCLUSIVE does not have a clear definition,"
               " "
      ...
      case HPCRUN_FMT_METRIC_SHOW_EXCLUSIVE:
        settings.scopes &=
            {MetricScope::lex_aware, MetricScope::function, MetricScope::point};
        break;
      case HPCRUN_FMT_METRIC_INVISIBLE:
        settings.visibility = Metric::Settings::visibility_t::invisible;
        break;
      default:
        util::log::verror{} << "Unknown show parameter "
                            << (unsigned int)m.flags.fields.show << ", " << m.name
  ...
  if (needed.hasContexts()) {
    ...
    while ((id = hpcrun_sparse_next_context(file, &n)) > 0) {
      ...
      const auto outlineGpuContext = [this](uint64_t context) {
        ...
        auto node =
            std::find_if(tuple.rbegin(), tuple.rend(), [](const auto& e) -> bool {
        ...
        tuple.push_back(
            {IDTUPLE_COMPOSE(IDTUPLE_GPUCONTEXT, IDTUPLE_IDS_LOGIC_LOCAL), context, 0});
      ...
      if (const auto* p_x = std::get_if<singleCtx_t>(&par)) {
        ...
        if (n.lm_id == HPCRUN_GPU_CONTEXT_NODE) {
          ...
          nodes.emplace(id, refRangeContext_t(
                                *p_x, sink.mergedThread(outlineGpuContext(n.lm_ip))));
          continue;
        }
        ...
        if (n.lm_id == HPCRUN_PLACEHOLDER_LM && n.unwound) {
          ...
          case hpcrun_placeholder_root_partial: // Because hpcrun stitches here
            if (&par.get() != &global) {
              util::log::info{} << "Encountered <partial root> that wasn't a child of "
                                   "<primary root>!";
              return false;
            }
            // fallthrough
          ...
          // Only certain placeholders can be used as the top-level Context (just below
          ...
          default:
            ...
            if (&par.get() == &global) {
              if (!warned_top_demotion) {
                util::log::vwarning()
                    << "Demoting invalid top-level Contexts to <partial call paths> in "
                    << path.filename().string();
                warned_top_demotion = true;
              }
              par = sink.context(global, {Relation::call, Scope()}).second;
              util::log::info()
                  << "Demoted invalid top-level cct (id " << id << "): " << scope;
            }
            break;
          }
        } else {
          ...
          if (&par.get() == &global) {
            ...
            par = sink.context(global, {Relation::call, Scope()}).second;
            util::log::info() << "Demoted invalid top-level cct (id " << id
                              << "): " << scope;
          }
        }
        ...
        if (!n.unwound) {
          ...
          nodes.emplace(
              id, singleCtx_t(par.get(), sink.context(par, {Relation::call, scope})));
        }
        ...
        if (n.lm_id != HPCRUN_GPU_RANGE_NODE) {
          ...
          util::log::info{}
              << "Encountered invalid non-GPU_RANGE child of GPU_CONTEXT node";
          return false;
        }
        // Add this root to the proper reconstruction group
        sink.addToReconstructionGroup(*p_x->first.par, p_x->first.rel.scope().flat(),
                                      p_x->second, n.lm_ip);
        ...
        if (n.lm_id != HPCRUN_GPU_CONTEXT_NODE) {
          ...
          util::log::info{} << "Encountered invalid non-GPU_CONTEXT child of outlined "
                               "range tree root";
          return false;
        }
        nodes.emplace(
            id, outlinedRangeContext_t{sink.mergedThread(outlineGpuContext(n.lm_ip))});
  ...
  if (needed.hasMetrics()) {
    ...
    while ((cid = hpcrun_sparse_next_block(file)) > 0) {
      ...
      if (node_it == nodes.end()) {
        util::log::info{} << "Encountered metric value for invalid cct node id: "
                          << cid;
        return false;
      }
        ...
        if (x.isRelation) {
          if (!raccum) {
            if (auto* p_x = std::get_if<singleCtx_t>(&node_it->second)) {
              ...
              raccum = sink.accumulateTo(p_x->first.second, p_x->second,
                                         p_x->first.first.rel);
            } else {
              util::log::info{}
                  << "Encountered metric value for cct node of invalid type: " << cid;
              return false;
            }
          }
          ...
          if (!faccum) {
            if (auto* p_x = std::get_if<singleCtx_t>(&node_it->second)) {
              ...
              faccum = sink.accumulateTo(*thread, p_x->ctx);
                           ...
                           std::get_if<outlinedRangeSample_t>(&node_it->second)) {
              faccum =
                  sink.accumulateTo(p_x->first.first, p_x->first.second, p_x->second);
            } else {
              util::log::info{}
                  << "Encountered metric value for cct node of invalid type: " << cid;
              return false;
      ...
      if (it != nodes.end()) {
        if (auto* p_x = std::get_if<singleCtx_t>(&it->second)) {
          switch (sink.timepoint(
              *thread, callTrace ? p_x->rel : p_x->full,
...
Expression Hpcrun4::parseFormulaPrimary(std::istream& s) const {
  ...
  case '(': {
    ...
    if (s.get() != ')')
      throw std::invalid_argument(
          "Attempt to parse invalid formula, parans not balanced");
    return e;
  }
  case '-': {
    s.get();
  ...
  case '#': {
    ...
    if (!s)
      throw std::invalid_argument(
          "Attempt to parse invalid formula, invalid variable ref");
  ...
  case 'm': {
    ...
    Expression::Kind op;
    if (funcname == "max")
      op = Expression::Kind::op_max;
    else if (funcname == "min")
      op = Expression::Kind::op_min;
    else
      throw std::invalid_argument(
          "Attempt to parse invalid formula, unrecognized function");
    if (s.get() != '(')
      throw std::invalid_argument(
          "Attempt to parse invalid formula, bad function syntax, no open parens");
    ...
    if (end != ')')
      throw std::invalid_argument(
          "Attempt to parse invalid formula, bad function syntax, no close parens");
...
Expression Hpcrun4::parseFormula1(std::istream& s, Expression lhs, int min_prec) const {
  ...
  while (lookahead && std::get<1>(*lookahead) >= min_prec) {
    ...
    while (lookahead &&
      ...
      rhs = parseFormula1(s, std::move(rhs),
                          op_prec + (std::get<1>(*lookahead) > op_prec ? 1 : 0));
      lookahead = peekFormulaOperator(s);
    }
