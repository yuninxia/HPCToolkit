...
static_assert(std::is_same_v<StatisticAccumulator::raw_t, std::array<double, 5>>,
              "unpack needs to be updated!");
template <>
StatisticAccumulator::raw_t
unpack<StatisticAccumulator::raw_t>(std::vector<uint8_t>::const_iterator& it) noexcept {
...
std::vector<uint8_t>::const_iterator Packed::unpackAttributes(iter_t it) noexcept {
  ...
  for (std::size_t i = 0; i < cnt; i++) {
    ...
    const std::function<Expression()> unpackExpression = [&]() -> Expression {
      ...
      case Expression::Kind::subexpression:
        assert(false &&
               "Attempt to unpack invalid Expression, should not have sub-Expressions");
        abort();
      default: {
  ...
  for (auto& [m, nparts] : metrics) {
    sink.metricFreeze(m);
    assert(m.get().partials().size() == nparts &&
           "Inconsistent number of partials across ranks!");
  }
  return it;
}
...
std::vector<uint8_t>::const_iterator Packed::unpackContexts(iter_t it) noexcept {
  std::stack<std::reference_wrapper<Context>,
             std::vector<std::reference_wrapper<Context>>>
      tip;
  // Format: <global> children... [sentinel]
  [[maybe_unused]] auto globalTy = unpack<std::uint64_t>(it);
  assert(globalTy == (std::uint64_t)Scope::Type::global &&
  ...
  while (1) {
    ...
    auto& c =
        sink.context(tip.empty() ? sink.global() : tip.top().get(), {Relation::call, s})
            .second;
    tip.push(c);
  }
