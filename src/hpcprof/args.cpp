...
ProfArgs::ProfArgs(int argc, char* const argv[])
    : title(), threads(0), output(), include_sources(true), include_traces(true),
      include_thread_local(true), format(Format::metadb),
      dwarfMaxSize(100 * 1024 * 1024), valgrindUnclean(false) {
  ...
  int opt;
  int longopt;
  while ((opt = getopt_long(argc, argv, "hVvqQO:o:j:S:R:n:f:M:", longopts, &longopt)) >=
         0) {
    switch (opt) {
    case 'h':
    ...
    case 'S': {
      ...
      try {
        c.reset(new finalizers::StructFile(
            path, fs::path(),
    ...
    case 'M': {
      const char* const options[] = {"none",    "sum",   "normal",
                                     "extrema", "stats", nullptr};
      char* value;
      while (optarg[0] != '\0') {
        switch (getsubopt(&optarg, const_cast<char* const*>(options), &value)) {
        case 0: // none
          stats.sum = stats.mean = stats.min = stats.max = stats.stddev = stats.cfvar =
              false;
          break;
        case 1: // sum
          stats.sum = true;
    ...
    case 0:
      ...
      case 0: { // --metric-db (mutually exclusive with --no-thread-local)
        if (seenNoThreadLocal) {
          std::cerr
              << "Error: --metric-db and --no-thread-local cannot be used together!\n";
          std::exit(2);
        }
      ...
      case 3: { // --only-exe
        ...
        if (!exe.has_filename()) {
          std::cerr
              << "WARNING: ignoring malformed argument to --only-exe, no filename: "
              << exe.native() << "\n";
          break;
        }
        if (exe.has_parent_path()) {
          std::cerr << "WARNING: ignoring leading path to --only-exe, will match on "
                       "filename: "
                    << exe.filename().generic_string() << "\n";
        }
    ...
    if (mpi::World::rank() == 0) {
        ...
        if (argc - optind == 1) { // == only one input file argument
          ...
          if (string_starts_with(fn, "hpctoolkit-")) {
            ...
            if (tail != std::string::npos &&
                (tail >= fn.size() - 13 ||
                 (fn[tail + 13] == '-' &&
                  std::all_of(&fn[tail + 14], &fn.back(), [](const char& c) {
      ...
      if (stdshim::filesystem::exists(output)) {
        if (arg_overwriteOutput == 0) {
          ...
          if (state == DEFAULT)
            state = DEFAULT_SUFFIXED;
          else
            state = EXPLICIT_SUFFIXED;
        } else {
      ...
      case EXPLICIT_SUFFIXED:
        util::log::argsinfo{}
            << "Specified database exists; writing analysis results to "
            << (output / "").native();
        break;
      }
    }
    ...
    if (fs::is_directory(p)) {
      ...
      if (!arg_ignore_structs) {
        ...
        if (fs::is_directory(sp)) {
          ...
          if (mpi::World::rank() == 0)
            recstore = std::make_shared<finalizers::StructFile::RecommendationStore>(
                true, std::move(structargs));
          ...
          util::call_once(onceMissingGPUCFGs, [&] {
            ...
            if (mpi::World::rank() == 0) {
              util::log::warning()
                  << "Program structure data is missing from the measurements "
                     "directory, performance attribution will be degraded\n"
...
#pragma omp for schedule(dynamic) nowait
      for (std::size_t i = 0; i < files.size(); i++) {
        ...
        if (!fs::is_directory(meas))
          meas = "";
        auto s = ProfileSource::create_for(pg.first, meas);
        if (!only_exes.empty()) {
          if (auto* r4 = dynamic_cast<hpctoolkit::sources::Hpcrun4*>(s.get());
              r4 != nullptr) {
      ...
      }
      {
        ...
        for (auto& sp : my_source_args)
          source_args.emplace_back(std::move(sp));
      }
  ...
  for (uint32_t i = 0; i < extra.size(); i++) {
    ...
    if (!s)
      util::log::fatal{}
          << "Input " << p
          << " has changed on disk, please let it stabilize before continuing!";
    sources.emplace_back(std::move(s), std::move(p));
    source_args.emplace_back(std::move(arg));
  }
}

static std::pair<bool, fs::path> remove_prefix(const fs::path& path,
                                               const fs::path& pre) {
...
search(const std::unordered_map<fs::path, fs::path, stdshim::hash_path>& prefixes,
...
std::optional<fs::path> ProfArgs::Prefixer::resolvePath(const File& f) noexcept {
  ...
  if (args.foreign &&
      std::none_of(args.allowedForeignDirs.begin(), args.allowedForeignDirs.end(),
                   [&](const auto& base) {
                     return is_subpath(result.value_or(f.path()), base);
                   })) {
...
std::optional<fs::path> ProfArgs::Prefixer::resolvePath(const Module& m) noexcept {
  ...
  if (args.foreign &&
      std::none_of(args.allowedForeignDirs.begin(), args.allowedForeignDirs.end(),
                   [&](const auto& base) {
                     return is_subpath(result.value_or(m.path()), base);
                   })) {
    // The path is not part of an allowed subtree in the foreign case.
    // Return an empty path, meaning the path does not exist on the current filesystem.
    return fs::path();
  ...
  if (fg.scope().type() == Scope::Type::point) {
    ...
    if (args.structpaths.find(m.path()) == args.structpaths.end()) {
      ...
      util::call_once(args.onceMissingGPUCFGs, [&] {
        if (mpi::World::rank() == 0) {
          util::log::warning() << "Control flow data is missing or corrupted, "
                                  "disabling affected GPU context reconstruction";
        }
      });
    }
  }
