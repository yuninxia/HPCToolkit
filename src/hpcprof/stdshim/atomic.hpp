...
public:
  ...
  void wait(std::uint32_t, std::memory_order = std::memory_order_seq_cst) const
      volatile noexcept;
...
public:
  ...
  constexpr atomic_nonint(T v) noexcept
      : atomic_uint32(static_cast<std::uint32_t>(v)){};
  ...
  bool compare_exchange_weak(T& e, T d, std::memory_order s,
                             std::memory_order f) noexcept {
    std::uint32_t ex = static_cast<std::uint32_t>(e);
    bool res =
        atomic_uint32::compare_exchange_weak(ex, static_cast<std::uint32_t>(d), s, f);
    e = static_cast<T>(ex);
    return res;
  }
  bool compare_exchange_weak(T& e, T d, std::memory_order s,
                             std::memory_order f) volatile noexcept {
    std::uint32_t ex = static_cast<std::uint32_t>(e);
    bool res =
        atomic_uint32::compare_exchange_weak(ex, static_cast<std::uint32_t>(d), s, f);
    e = static_cast<T>(ex);
  ...
  bool compare_exchange_weak(T& e, T d,
    ...
    std::uint32_t ex = static_cast<std::uint32_t>(e);
    bool res =
        atomic_uint32::compare_exchange_weak(ex, static_cast<std::uint32_t>(d), o);
    e = static_cast<T>(ex);
    return res;
  }
  ...
  bool
  compare_exchange_strong(T& e, T d,
                          std::memory_order o = std::memory_order_seq_cst) noexcept {
    std::uint32_t ex = static_cast<std::uint32_t>(e);
    bool res =
        atomic_uint32::compare_exchange_weak(ex, static_cast<std::uint32_t>(d), o);
  ...
  void wait(T v, std::memory_order o = std::memory_order_seq_cst) const
      volatile noexcept {
    atomic_uint32::wait(static_cast<std::uint32_t>(v), o);
  }
...
struct is_futex_int_helper<T, true>
    : std::integral_constant<bool, std::numeric_limits<T>::radix == 2 &&
                                       (std::numeric_limits<T>::digits +
                                        (std::is_signed_v<T> ? 1 : 0)) <= 32> {};
template <class T> using is_futex_int = is_futex_int_helper<T, std::is_integral_v<T>>;
...
using atomic = typename std::conditional<
    std::is_same<T, std::uint32_t>::value, detail::atomic_uint32,
    typename std::conditional<detail::is_futex_int<T>::value, detail::atomic_int<T>,
                              typename std::conditional<detail::is_futex_enum<T>::value,
                                                        detail::atomic_nonint<T>,
                                                        std::atomic<T>>::type>::type>::
    type;
