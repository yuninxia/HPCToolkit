...
template <class InIt>
constexpr inline typename iterator_traits<InIt>::value_type reduce(InIt first,
                                                                   InIt last) {
...
constexpr T transform_reduce(InIt1 first1, InIt1 last1, InIt2 first2, T init,
                             BinOp1 op1, BinOp2 op2) {
  if constexpr (is_random_access_iter<InIt1>::value &&
                is_random_access_iter<InIt2>::value) {
...
constexpr OutIt inclusive_scan(InIt first, InIt last, OutIt result, BinOp op) {
  if (first != last) {
    ...
    *result++ = init;
    ++first;
    if (first != last)
      result = hpctoolkit::stdshim::inclusive_scan(first, last, result, op,
                                                   ::std::move(init));
  }
  return result;
}
...
constexpr OutIt transform_inclusive_scan(InIt first, InIt last, OutIt result, BinOp op,
                                         UnOp trans, T init) {
  for (; first != last; ++first)
...
constexpr OutIt transform_inclusive_scan(InIt first, InIt last, OutIt result, BinOp op,
  ...
  if (first != last) {
    ...
    *result++ = init;
    ++first;
    if (first != last)
      result = hpctoolkit::stdshim::transform_inclusive_scan(first, last, result, op,
                                                             trans, ::std::move(init));
  }
  return result;
}
...
constexpr OutIt exclusive_scan(InIt first, InIt last, OutIt result, T init) {
  return hpctoolkit::stdshim::exclusive_scan(first, last, result, ::std::move(init),
                                             ::std::plus<>{});
}

template <class InIt, class OutIt, class T, class BinOp, class UnOp>
constexpr OutIt transform_exclusive_scan(InIt first, InIt last, OutIt result, T init,
                                         BinOp op, UnOp trans) {
  while (first != last) {
    auto v = init;
    init = op(init, trans(*first));
