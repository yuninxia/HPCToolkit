...
public:
  ...
  bool has(MetricScope s) const noexcept {
    return ((base&)*this)[static_cast<size_t>(s)];
  }
  auto operator[](MetricScope s) noexcept {
    return ((base&)*this)[static_cast<size_t>(s)];
  }

  using base::size;

  MetricScopeSet operator|(const MetricScopeSet& o) const {
...
private:
  ...
  struct RGroup {
    ...
    std::mutex lock;
    std::unordered_map<Scope, std::unordered_set<util::reference_index<Context>>>
        c_entries;
    std::unordered_map<
...
public:
  ...
  static raw_t rawZero() noexcept { return raw_t{0, 0, 0, 0, 0}; }
...
public:
  ...
  public:
    ...
    std::optional<double> get(MetricScope ms) const noexcept { return partial.get(ms); }
    ...
    // MT: Safe (const), Unstable (before `metrics` wavefront)
    raw_t getRaw() const noexcept { return partial.getRaw(); }
  ...
  private:
    friend class StatisticAccumulator;
    PartialCRef(const Partial& p) : partial(p){};
