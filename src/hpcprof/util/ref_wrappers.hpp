...
public:
  ...
  template <class U>
  friend constexpr bool operator==(const reference_index<U>& a,
                                   const reference_index<U>& b) noexcept;
...
template <class T>
constexpr bool operator==(const reference_index<T>& a,
                          const reference_index<T>& b) noexcept {
  return &a.get() == &b.get();
}
...
template <class T>
constexpr bool operator==(const reference_index<T>& a, T* b) noexcept {
  return &a.get() == b;
}
template <class T>
constexpr bool operator==(T* a, const reference_index<T>& b) noexcept {
  return a == &b.get();
}
template <class T>
constexpr bool operator!=(const reference_index<T>& a,
...
public:
  ...
  template <class U>
  friend constexpr bool operator==(const optional_ref<U>& a,
                                   const optional_ref<U>& b) noexcept;
...
public:
  ...
  template <class T, std::enable_if_t<in_pack<T, Ts...> && is_standard_ref<T>,
                                      std::nullptr_t> = nullptr>
  constexpr variant_ref(T& v) : d(reference_index<T>(v)){};
  template <class X, class Y,
            std::enable_if_t<in_pack<ref_pair<X, Y>, Ts...>, std::nullptr_t> = nullptr>
  ...
  constexpr variant_ref(
      std::pair<std::reference_wrapper<X>, std::reference_wrapper<Y>> v)
      : d(std::make_pair(reference_index<X>(v.first), reference_index<Y>(v.second))){};

  template <std::size_t I, class T,
            std::enable_if_t<is_standard_ref<T>, std::nullptr_t> = nullptr>
  ...
  template <std::size_t I, class X, class Y>
  constexpr variant_ref(std::in_place_index_t<I> i,
                        std::pair<reference_index<X>, reference_index<Y>> v)
      : d(i, v){};
...
private:
  ...
  template <class X, class Y> struct ref_add_const<ref_pair<X, Y>> {
    using type =
        ref_pair<typename ref_add_const<X>::type, typename ref_add_const<Y>::type>;
  };
...
private:
  template <class T, std::size_t I> T convert() const {
  ...
  struct const_compat_1
      : std::bool_constant<
            ...
            (std::is_const_v<T> ? std::is_const_v<N> : true) && is_standard_ref<T> &&
            is_standard_ref<N>> {};
...
public:
  ...
  template <class T>
  constexpr variant_ref
  operator=(std::enable_if_t<in_pack<T, Ts...> && is_standard_ref<T>, T&> v) {
  ...
  constexpr bool valueless_by_exception() const noexcept {
    return d.valueless_by_exception();
  }
  void swap(variant_ref& o) { d.swap(o.d); }

  template <class... Us>
  friend constexpr bool operator==(const variant_ref<Us...>& a,
                                   const variant_ref<Us...>& b) noexcept;
  template <class... Us>
  friend constexpr bool operator!=(const variant_ref<Us...>& a,
                                   const variant_ref<Us...>& b) noexcept;
  template <class... Us>
...
private:
  ...
  template <class T>
  constexpr std::enable_if_t<!std::is_same_v<T, std::monostate>, T&>
  std_get() const noexcept {
  ...
  template <class X, class Y, class... Ns>
  friend constexpr std::optional<std::pair<hpctoolkit::util::reference_index<X>,
                                           hpctoolkit::util::reference_index<Y>>>
  std::get_if(const variant_ref<Ns...>& v) noexcept;
  template <class X, class Y>
...
template <class... Ts>
constexpr bool operator==(const variant_ref<Ts...>& a,
                          const variant_ref<Ts...>& b) noexcept {
  return a.d == b.d;
}
template <class... Ts>
constexpr bool operator!=(const variant_ref<Ts...>& a,
...
public:
  ...
  operator()(const hpctoolkit::util::optional_ref<T>& v) const noexcept {
    return realhash(v);
  }
};
...
template <class X, class Y>
class hash<std::pair<hpctoolkit::util::reference_index<X>,
                     hpctoolkit::util::reference_index<Y>>> {
...
public:
  constexpr std::size_t
  operator()(const std::pair<hpctoolkit::util::reference_index<X>,
...
public:
  constexpr std::size_t
  operator()(const hpctoolkit::util::variant_ref<Ts...>& v) const noexcept {
    return realhash(v.d);
  }
};
...
template <class T, class... Ts>
constexpr bool
holds_alternative(const hpctoolkit::util::variant_ref<Ts...>& v) noexcept {
...
template <class X, class Y, class... Ts>
constexpr std::optional<std::pair<hpctoolkit::util::reference_index<X>,
                                  hpctoolkit::util::reference_index<Y>>>
get_if(const hpctoolkit::util::variant_ref<Ts...>& v) noexcept {
