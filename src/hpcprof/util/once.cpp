...
Once::Caller Once::signal() {
  std::thread::id old;
  [[maybe_unused]] auto ok = callerId.compare_exchange_strong(
      old, std::this_thread::get_id(), std::memory_order_acquire,
...
void OnceFlag::call_detail(bool block, const std::function<void(void)>& f) {
  ...
  while (true) {
    ...
    case Status::inprogress:
      ...
      break;
    case Status::initial:
      if (status.compare_exchange_weak(s, Status::inprogress,
