...
void File::synchronize() noexcept {
  ...
  if (mpi::World::rank() == 0) {
    ...
    impl->fd =
        ::open(impl->path.c_str(), O_RDWR | (impl->create ? O_CREAT | O_TRUNC : 0),
...
void File::remove() noexcept {
  assert(impl && impl->fd != -1 &&
         "Attempt to call File::remove before File::synchronize!");
  if (mpi::World::rank() == 0) {
...
void File::Instance::readat(std::uint_fast64_t offset, std::size_t size,
                            char* buf) noexcept {
...
void File::Instance::writeat(std::uint_fast64_t offset, std::size_t size,
  ...
  while (size > 0) {
    ...
    if (cnt < 0) {
      char buf[1024];
      util::log::fatal{} << "Error during write: "
                         << strerror_r(errno, buf, sizeof buf);
    } else if (cnt == 0) {
