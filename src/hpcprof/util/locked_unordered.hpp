...
template <class M>
static auto
SharedMutex_helper(int) -> sfinae_true<decltype(std::declval<M>().lock_shared()),
...
template <class M>
using su_lock = typename std::conditional<SharedMutex<M>::value, std::shared_lock<M>,
                                          std::unique_lock<M>>::type;
} // namespace

/// A simple parallel wrapper around a std::unordered_map.
template <class K, class V, class M = stdshim::shared_mutex, class H = std::hash<K>,
...
private:
  ...
  std::pair<V&, bool> opget(std::shared_lock<Mtx>&& l, KA&& k, Args&&... args) {
    ...
    return opget(std::unique_lock<Mtx>(lock), std::forward<KA>(k),
