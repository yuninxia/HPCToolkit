...
static lzma_ret maybeThrowLZMA_decoder(lzma_ret ret) {
  ...
  case LZMA_MEMLIMIT_ERROR:
    throw std::runtime_error(
        "LZMA decoder hit memory limit (the impossible happened?)");
  ...
  case LZMA_BUF_ERROR:
    throw std::runtime_error("LZMA decoder failed (multiple times) to make progress "
                             "(4K is too small a buffer size?)");
...
lzmastreambuf::int_type lzmastreambuf::underflow() {
  ...
  if (gptr() == egptr()) {
    ...
    if (!tail) {
      ...
      stream.avail_in +=
          base->sgetn(in_buffer + stream.avail_in, BUFSIZE - stream.avail_in);
    }
