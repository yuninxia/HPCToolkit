...
void IdPacker::notifyWavefront(DataClass ds) {
  if (ds.hasContexts() && ds.hasAttributes()) { // This is it!
    ...
    // Format: [global id] [cnt] ([parent id] [nonce] [cnt] ([child hash] [child
    // id])...)...
    ...
    src.contexts().citerate(
        [&](const Context& c) {
          ...
          std::vector<std::tuple<util::stable_hash_state, std::uint64_t, unsigned int,
                                 std::reference_wrapper<const Context>>>
              children;
          for (const Context& cc : c.children().citerate()) {
            util::stable_hash_state hashstate;
            hashstate << cc.scope();
            children.emplace_back(std::move(hashstate), 0,
                                  cc.userdata[src.identifier()], cc);
          }
