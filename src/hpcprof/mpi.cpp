...
// Many MPI functions use `int` as the size of the data to send, however we use bytes
// and `size_t` instead. This template helps break these calls down into smaller pieces
// that MPI can handle.
template <class F>
...
template <class F>
static void segment(void* data1, void* data2, std::size_t cnt, const Datatype& ty,
                    F&& f) {
  for (std::size_t off = 0; off < cnt; off += std::numeric_limits<int>::max()) {
    int sz = std::min<std::size_t>(cnt - off, std::numeric_limits<int>::max());
...
template <class F>
static void segment(const void* data1, const void* data2, std::size_t cnt,
                    const Datatype& ty, F&& f) {
  for (std::size_t off = 0; off < cnt; off += std::numeric_limits<int>::max()) {
    int sz = std::min<std::size_t>(cnt - off, std::numeric_limits<int>::max());
    f((const void*)((const char*)data1 + off * ty.sz),
      (const void*)((const char*)data2 + off * ty.sz), sz);
  }
}
...
void detail::reduce(void* data, std::size_t cnt, const Datatype& ty, std::size_t root,
                    const Op& op) {
  SmallMem send(cnt * ty.sz);
  std::memcpy(send, data, cnt * ty.sz);
  auto l = mpiLock();
  segment(send, data, cnt, ty, [&](void* send, void* data, int cnt) {
    if (MPI_Reduce(send, data, cnt, ty.value, static_cast<const BaseOp&>(op).op, root,
...
void detail::allreduce(void* data, std::size_t cnt, const Datatype& ty, const Op& op) {
  ...
  auto l = mpiLock();
  segment(send, data, cnt, ty, [&](void* send, void* data, int cnt) {
    if (MPI_Allreduce(send, data, cnt, ty.value, static_cast<const BaseOp&>(op).op,
                      MPI_COMM_WORLD) != MPI_SUCCESS)
...
void detail::scan(void* data, std::size_t cnt, const Datatype& ty, const Op& op) {
  ...
  auto l = mpiLock();
  segment(send, data, cnt, ty, [&](void* send, void* data, int cnt) {
    if (MPI_Scan(send, data, cnt, ty.value, static_cast<const BaseOp&>(op).op,
                 MPI_COMM_WORLD) != MPI_SUCCESS)
...
void detail::exscan(void* data, std::size_t cnt, const Datatype& ty, const Op& op) {
  ...
  auto l = mpiLock();
  segment(send, data, cnt, ty, [&](void* send, void* data, int cnt) {
    if (MPI_Exscan(send, data, cnt, ty.value, static_cast<const BaseOp&>(op).op,
                   MPI_COMM_WORLD) != MPI_SUCCESS)
...
void detail::gather_root(void* recv, std::size_t cnt, const Datatype& ty,
                         std::size_t rootRank) {
  assert(World::rank() == rootRank &&
         "mpi::detail::gather_root is only valid at the root!");
  assert(cnt <= std::numeric_limits<int>::max() &&
         "attempt to call mpi::detail::gather_root with a large cnt");
  SmallMem send(cnt * ty.sz);
  std::memcpy(send, (char*)recv + cnt * ty.sz * rootRank, cnt * ty.sz);
  auto l = mpiLock();
  if (MPI_Gather(send, cnt, ty.value, recv, cnt, ty.value, rootRank, MPI_COMM_WORLD) !=
      MPI_SUCCESS)
...
void detail::gather(void* send, std::size_t cnt, const Datatype& ty,
  ...
  assert(cnt <= std::numeric_limits<int>::max() &&
         "attempt to call mpi::detail::gather with a large cnt");
  auto l = mpiLock();
...
void detail::gatherv_root(void* recv, const std::size_t* cnts, const Datatype& ty,
                          std::size_t rootRank) {
  assert(World::rank() == rootRank &&
         "mpi::detail::gatherv_root is only valid at the root!");
  ...
  for (std::size_t i = 0, idx = 0; i < World::size(); i++) {
    assert(idx <= std::numeric_limits<int>::max() &&
           "attempt to call mpi::detail::gatherv_root with a large cnt");
    icnts[i] = cnts[i];
    ioffsets[i] = idx;
    idx += cnts[i];
  }
...
void detail::gatherv(void* send, std::size_t cnt, const Datatype& ty,
  ...
  assert(cnt <= std::numeric_limits<int>::max() &&
         "attempt to call mpi::detail::gatherv with a large cnt");
  auto l = mpiLock();
  if (MPI_Gatherv(send, cnt, ty.value, nullptr, nullptr, nullptr, ty.value, rootRank,
                  MPI_COMM_WORLD) != MPI_SUCCESS)
...
void detail::scatter_root(void* send, std::size_t cnt, const Datatype& ty,
                          std::size_t rootRank) {
  assert(World::rank() == rootRank &&
         "mpi::detail::scatter_root is only valid at the root!");
  assert(cnt <= std::numeric_limits<int>::max() &&
         "attempt to call mpi::detail::scatter_root with a large cnt");
...
void detail::scatter(void* data, std::size_t cnt, const Datatype& ty,
  ...
  assert(cnt <= std::numeric_limits<int>::max() &&
         "attempt to call mpi::detail::scatter with a large cnt");
  auto l = mpiLock();
...
void detail::scatterv_root(void* send, const std::size_t* cnts, const Datatype& ty,
                           std::size_t rootRank) {
  assert(World::rank() == rootRank &&
         "mpi::detail::scatterv_root is only valid at the root!");
  std::vector<int> icnts(World::size());
  std::vector<int> ioffsets(World::size());
  for (std::size_t i = 0, idx = 0; i < World::size(); i++) {
    assert(idx <= std::numeric_limits<int>::max() &&
           "attempt to call mpi::detail::scatterv_root with a large cnt");
    icnts[i] = cnts[i];
    ioffsets[i] = idx;
    idx += cnts[i];
  }
  ...
  auto l = mpiLock();
  if (MPI_Scatterv(send, icnts.data(), ioffsets.data(), ty.value, recv, icnts[rootRank],
                   ty.value, rootRank, MPI_COMM_WORLD) != MPI_SUCCESS)
...
void detail::scatterv(void* data, std::size_t cnt, const Datatype& ty,
                      std::size_t rootRank) {
  assert(World::rank() != rootRank &&
         "mpi::detail::scatterv is not valid at the root!");
  assert(cnt <= std::numeric_limits<int>::max() &&
         "attempt to call mpi::detail::scatterv with a large cnt");
  auto l = mpiLock();
  if (MPI_Scatterv(nullptr, nullptr, nullptr, ty.value, data, cnt, ty.value, rootRank,
                   MPI_COMM_WORLD) != MPI_SUCCESS)
    util::log::fatal{}
        << "Error while performing an MPI vectorized scatter (non-root)!";
}

void detail::send(const void* data, std::size_t cnt, const Datatype& ty, Tag tag,
                  std::size_t dst) {
  auto l = mpiLock();
  segment(data, cnt, ty, [&](const void* data, int cnt) {
    if (MPI_Send(data, cnt, ty.value, dst, static_cast<int>(tag), MPI_COMM_WORLD) !=
        MPI_SUCCESS)
      util::log::fatal{} << "Error while performing an MPI send!";
  });
}

void detail::recv(void* data, std::size_t cnt, const Datatype& ty, Tag tag,
                  std::size_t src) {
  auto l = mpiLock();
  segment(data, cnt, ty, [&](void* data, int cnt) {
    if (MPI_Recv(data, cnt, ty.value, src, static_cast<int>(tag), MPI_COMM_WORLD,
...
std::optional<std::size_t> detail::recv_server(void* data, std::size_t cnt,
  ...
  assert(cnt <= std::numeric_limits<int>::max() &&
         "attempt to call mpi::detail::recv_server with a large cnt");
  ...
  if (l) {
    ...
    MPI_Request req;
    if (MPI_Irecv(data, cnt, ty.value, MPI_ANY_SOURCE, static_cast<int>(tag),
    ...
    if (MPI_Recv(data, cnt, ty.value, MPI_ANY_SOURCE, static_cast<int>(tag),
...
void detail::cancel_server(const Datatype& ty, Tag tag) {
  auto l = mpiLock();
  if (MPI_Send(nullptr, 0, ty.value, World::rank(), static_cast<int>(tag),
               MPI_COMM_WORLD) != MPI_SUCCESS)
