...
RankTree::RankTree(std::size_t arity)
    : arity(std::max<std::size_t>(arity, 1)),
      parent(mpi::World::rank() > 0 ? (mpi::World::rank() - 1) / arity
                                    : (std::size_t)-1),
...
Receiver::Receiver(std::vector<std::uint8_t>& block) : readBlock(true), block(block){};
...
MetricSender::MetricSender(RankTree& t) : sinks::ParallelPacked(false, true), tree(t){};
...
util::WorkshareResult MetricSender::help() { return helpPackMetrics(); }
