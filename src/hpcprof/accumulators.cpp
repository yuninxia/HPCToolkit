...
StatisticAccumulator::PartialRef
StatisticAccumulator::get(const StatisticPartial& p) noexcept {
  return {partials[p.m_idx], p};
}

void MetricAccumulator::add(double v) noexcept { atomic_add(point, v); }
...
std::optional<double> StatisticAccumulator::Partial::get(MetricScope s) const noexcept {
  ...
  case MetricScope::function:
    return opt0(function.load(std::memory_order_relaxed));
  case MetricScope::lex_aware:
    return opt0(isLoop ? function_noloops.load(std::memory_order_relaxed)
                       : function.load(std::memory_order_relaxed));
  case MetricScope::execution:
    return opt0(execution.load(std::memory_order_relaxed));
  };
  assert(false && "Invalid MetricScope!");
  std::abort();
}

util::optional_ref<const StatisticAccumulator>
Metric::getFor(const Context& c) const noexcept {
...
std::optional<double> MetricAccumulator::get(MetricScope s) const noexcept {
  ...
  case MetricScope::lex_aware:
    return opt0(isLoop ? function_noloops : function);
  case MetricScope::execution:
...
StatisticAccumulator& PerContextAccumulators::statisticsFor(const Metric& m) noexcept {
  return m_statistics
      .emplace(std::piecewise_construct, std::forward_as_tuple(m),
...
void PerThreadTemporary::finalize() noexcept {
  ...
  // attributed to Reconstructions and FlowGraphs within this Thread.
  {
    ...
    for (const auto& [r, input] : r_data.citerate()) {
      auto [factors, hasEC] = r->rescalingFactors(c_data);
      {
        ...
        std::transform(factors.begin(), factors.end(), inFs.cbegin(), factors.begin(),
    ...
    for (auto& [idx, group] : r_groups.iterate()) {
      for (const auto& [fg_c, input] : group.fg_data.citerate()) {
        ...
        for (auto& [r, factors] : std::move(exFactors)) {
          ...
          std::transform(factors.begin(), factors.end(), rsFs.cbegin(), factors.begin(),
                         std::multiplies<double>{});
  ...
  for (const auto& cx : c_data.citerate()) {
    ...
    if (!c.get().direct_parent()) {
      ...
      assert(c.get().scope().flat().type() == Scope::Type::global &&
  ...
  struct frame_t {
    ...
    std::vector<std::pair<std::reference_wrapper<const Context>,
                          std::reference_wrapper<const md_t>>>
        submds;
  };
  ...
  while (!stack.empty()) {
    ...
    for (auto& mx : data.iterate()) {
      ...
      mx.second.execution = mx.second.function = mx.second.function_noloops =
    ...
    for (std::size_t i = 0; i < stack.top().submds.size(); i++) {
      ...
      const bool pullNoLoops =
          sub.first.get().scope().flat().type() != Scope::Type::lexical_loop &&
    ...
    for (const auto& mx : data.citerate()) {
      ...
      auto& accum =
          cdata
              .emplace(std::piecewise_construct, std::forward_as_tuple(mx.first),
