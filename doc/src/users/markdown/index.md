...
HPCToolkit provides the `hpcviewer` ([Adhianto, Mellor-Crummey, and Tallent 2010](https://doi.org/10.1109/ICPPW.2010.35); [Tallent et al. 2011](https://doi.acm.org/10.1145/1995896.1995908)) graphical user interface for interactive examination of performance databases.
...
the display of two dimensions at a time: the [*Profile View*](#sec:profile) displays
pairs of (call-path, metric) or (execution context,
metric) dimensions and the [*Trace View*](#sec:trace) visualizes the
behavior of execution contexts over time.
The table below summarizes views supported by `hpcviewer`.

| View                  | Dimension                    | Note                                                                |
| :-------------------- | :--------------------------- | :------------------------------------------------------------------ |
| Profile - [Metric view](#sec:pane-metric)            | Call-path x Metrics  | display the tree and its associated metrics                         |
| Profile - [Thread view](#sec:hpcviewer:thread-level) | Call-path x Metrics  | display the tree and its metrics for a set of execution contexts    |
| Profile - [Graph view](#sec:hpcviewer:plots)         | Execution contexts x Metric | display a metric of a specific tree node for all execution contexts |
| Trace - [Main view](#sec:mainview)                     | Execution contexts x Time   | display execution context behavior over time                        |
| Trace - [Depth view](#sec:depthview)    | Call-path x Time     | display call stacks over time of an execution context               |

Note that in the [Profile view](#sec:profile), GPU stream execution contexts are not shown in this view; metrics for a GPU operation are associated with the calling context in the thread that initiated the GPU operation (Section [Thread View](sec:hpcviewer:thread-level)).
In the [Trace view](sec:trace), GPU streams have their trace lines independently from their host, allowing for the traces between hosts and devices to be separated.
...
- **New window**\
  ...
  However, filtering CCT node operation (see [Filtering Tree Nodes](#sec:filter)) will also affect all `hpcviewer` windows.
- **Open database**\
...
- **Open remote database** *(experimental feature)* \
  ...
  See [Remote database](sec:remote) page for further details.
- **Switch database** \
...
- **Filter CCT nodes** \
  Open a filter property window that lists a set of filters and their properties (Section [Filtering Tree Nodes](#sec:filter)).
...
- **Show color map** *(Trace view only)* \
  Open a window showing the customized mapping between a procedure pattern and a color. See the [Color map](#sec:color-map) Section.
