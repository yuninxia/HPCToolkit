...
The symbol <span class="large-text">»</span> represents a call to a procedure. For instance, the following node means that at line `1234` there is a call to function `foo()`:

> `1234` <span class="large-text">»</span> `foo()`
...
- **Flatten** ![image](icons/list-hide-left.svg) /
  **Unflatten** ![image](icons/list-unhide-left.svg)
  (Flat View only):
...
- **Zoom-in** ![image](icons/list-hide-above.svg) /
  **Zoom-out** ![image](icons/list-unhide-above.svg) :
...
- **Hot call path** ![image](icons/fire2-symbolic.svg) :
...
- **Add derived metric** ![image](icons/column-add.svg) :
...
- **Hide/show metrics** ![image](icons/eye-open-negative-filled-symbolic.svg) :
...
- **Resizing metric columns** ![image](icons/column-shrink.svg)/
  ![image](icons/column-grow.svg) :
  Resize the metric columns based on either the width of the data or the width of both of the data and the column's label.

- **Export into a CSV format file** ![image](icons/share-symbolic.svg) :
...
- **Increase font size** ![image](icons/loupe-plus-large-symbolic.svg) /
  **Decrease font size** ![image](icons/loupe-minus-large-symbolic.svg) :
  Increase or decrease the size of the navigation and metric panes.

- **Show a graph of metric values** ![image](icons/graph2-symbolic.svg) (Top-down view only):
...
- **Show the metrics of a set of threads** ![image](icons/tab-new-filled-symbolic.svg) (Top-down view only):
...
(fig:hpcviewer-derived-dialog-box)=
...
(fig:hpcviewer-view-scatterplot)=
...
Then, click the graph button ![image](icons/graph2-symbolic.svg) to show the associated sub-menus.
...
In a graph of metrics for a calling context, hovering over a dot in the graph will display the metric value and the name of its associated execution context.
In the Profile view, metrics for a GPU operation are attributed to either the main thread under `<program root>` or aggregated under `<thread root>` for operations launched by a thread with an id > 0. In the graphical thread view, each thread displays its value for the GPU metric associated with the calling context.
...
To select a set of execution contexts, one needs to use the execution-context selection window by clicking ![image](icons/tab-new-filled-symbolic.svg) button from the [control panel](#sec:control-panel) in the Top-down view.

(fig:hpcviewer-dialog-thread-select)=
...
(fig:hpcviewer-view-thread-level)=

| Threads view contains a CCT and metrics of a set of threads|
| :--------------------------------------------------------: |
|![Threads view](images/hpcviewer-thread-table.png)|
...
| :---------------: | :--------------------: |
|![Original tree](images/hpcviewer-filter-cct.png)|![Filter self](images/hpcviewer-filter-self.png)|
| The original top-down tree (or CCT) without any filters. | (fig:filter-self)= The result of applying *self only* filter on node `C` from CCT in Figure [Original CCT](fig:filter-cct). Node `C` is elided and its children (nodes `D` and `E`) are augmented to the parent of node `C`. The yellow color shows the new exclusive cost of node `A`.|
...
| :-------------------------------: | :----------------------------------------: |
|(fig:filter-children)=
![Filter descendants](images/hpcviewer-filter-children.png) |![Filter self and descendants](images/hpcviewer-filter-self_children.png)(fig:filter-selfchildren)= |
...
- **Hiding or showing metric columns**.
  ...
  The icon ![image](icons/eye-open-negative-filled-symbolic.svg) above the metric pane will bring up the metric property pane on the source pane area.
