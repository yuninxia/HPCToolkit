...
amd64) apt_packages+=(nvidia-cuda-toolkit libnvidia-compute-550) ;;
esac
...
if ! (for pkg in "${apt_packages[@]}"; do dpkg-query -s "$pkg" || exit 1; done) &>/dev/null; then
  ...
  DEBIAN_FRONTEND=noninteractive apt-get upgrade --update --with-new-pkgs --yes --no-install-recommends "${apt_packages[@]}" || exit $?
...
python3 -m pip install --break-system-packages 'meson>=1.1.0,<2' || exit $?
